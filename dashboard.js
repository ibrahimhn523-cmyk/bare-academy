...
/* ── تغيير الحالة: optimistic ثم كتابة ثم rollback عند الفشل ── */
...
function renderComm() {
  ...
  body.innerHTML = `
    <div class="filter-bar" style="margin-bottom:14px;flex-wrap:wrap">
      <input type="text" id="comm-search" placeholder="🔍 بحث بالاسم" oninput="renderCommTable()"
        style="flex:1;min-width:180px;padding:8px 12px;font-family:inherit;font-size:.9rem;background:var(--white);border:1px solid var(--border);border-radius:8px;color:var(--text)">
    ...
    <div class="comm-table-wrap">
      <table class="tbl">
        <thead><tr>
          <th>الاسم</th><th>الجوال</th><th>المجموعة</th><th>حالة الدفع</th><th>المتبقي</th><th style="width:60px">إجراء</th>
        </tr></thead>
...
function renderCommTable() {
  ...
  let d = [..._progSubs];
  if (q)   d = d.filter(s => (s.studentName || '').toLowerCase().includes(q));
  ...
  tbody.innerHTML = d.map(s => {
    ...
    return `<tr>
      ...
      <td class="td-actions"><button class="btn btn-sm btn-primary" style="background:#25d366" onclick="openWhatsApp(${s.id})" title="إرسال واتساب">📱</button></td>
    </tr>`;
  }).join('');
}

/* إدراج نص عند موضع المؤشر داخل أي textarea — قابل لإعادة الاستخدام (يستخدمه مودال القوالب) */
