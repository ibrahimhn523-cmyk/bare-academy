...
```
bare-academy/
├── index.html            (1377 سطر) — الواجهة العامة + نموذج التسجيل + طبقة عرض اليوم الوطني 96 (ADR-015)
...
├── fonts/                — خط TheYearofHandicrafts (5 أوزان)؛ و fonts/nd96/ خطا هوية اليوم الوطني (خط السعودية، IBM Plex Sans Arabic)
├── nd96/                 — أصول عرض اليوم الوطني 96: الشعار الرسمي (مرسوم من Logo.pdf) ولوحة «الكرم»
└── .claude/
...
### ADR-015 — عرض اليوم الوطني 96 على الصفحة الرئيسة: طبقة قابلة للإطفاء + توسيع قيد `period`
- **التاريخ:** 2026-09-21
- **السياق:**
  - عرض اليوم الوطني 96 (شهر 296، شهران 596، ثلاثة أشهر 896) صُمّم وجُرّب في `index96.html` بهوية اليوم الوطني الرسمية («عزّنا بطبعنا»)، واعتمده إبراهيم للنشر على الرابط الأساسي.
  - جدول `interests` كان يقبل في `period` القيم `first/second/both` فقط، فلم يكن هناك مكان لحفظ الباقة.
- **القرار:**
  1. **طبقة واحدة قابلة للإطفاء في `index.html`:** كل تعديلات العرض تحت `html.nd96`، ويتحكم بها الكائن `ND96` في `<head>`:
     - `ND96.enabled = false` يطفئ العرض فوراً.
     - `ND96.endsAt = 2026-09-30T23:59:59+03:00` انتهاء ذاتي؛ عند الصفر تعاد تحميل الصفحة فتعود كالإنتاج، بلا خطوط العرض ولا صوره.
  2. **الحفظ:** توسيع القيد `period_valid` ليقبل `nd96_m1` و`nd96_m2` و`nd96_m3` (DDL يدوي، والملف المحلي `db/interests-nd96-periods.sql`). التسجيل أثناء العرض يرسل `period` = مفتاح الباقة، و`program` = «الفصل الدراسي الأول 1448 · عرض اليوم الوطني 96». لا حقول جديدة في الطلب.
  3. **لوحة التحكم:** أسماء الباقات في `INT_PERIOD_AR` وفي فلتر الفترة.
  4. **الهوية:** الشعار الرسمي مرسوم آلياً من `Logo.pdf` بلا تعديل، و«خط السعودية» مع IBM Plex Sans Arabic في `fonts/nd96/`، ولوحة «الكرم» في `nd96/`. Helvetica Neue الموجود ضمن ملفات الهوية لم يُرفع لأن رخصته تجارية.
  5. **`index96.html` حُذف**، و`/index96.html` يُحوَّل إلى `/` في `vercel.json`.
- **البدائل المرفوضة:**
  - (أ) ربط الباقات بالفترات الحالية بلا DDL: كانت لوحة التحكم ستعرض باقة الشهرين «برنامجاً كاملاً».
  - (ب) عمود `package` جديد: تعديل أكبر بلا حاجة.
  - (ج) إبقاء صفحة العرض مستقلة بجانب الرئيسة: نسختان تختلفان مع الوقت.
- **النتيجة:**
  - تسجيلات العرض تُحفظ بباقتها، والتسجيل العادي بعد انتهاء العرض لم يتغير.
  - طبقة العرض تبقى في الكود خاملة بعد 30 سبتمبر إلى أن تُحذف بقرار لاحق.
  - **تنبيه قائم:** نص السياسة لا يذكر هدية التيشرت المعروضة في بطاقات الباقات.
...
| F24 | **عرض اليوم الوطني 96 على الصفحة الرئيسة** (ADR-015): `index.html` صار صفحة العرض بطبقة قابلة للإطفاء تنتهي ذاتياً 2026-09-30 (مُدّدت من 29 سبتمبر)، والتسجيل يحفظ الباقة في `interests.period` (`nd96_m1/m2/m3`) بعد توسيع القيد، ولوحة التحكم تعرض أسماء الباقات، و`/index96.html` يُحوَّل إلى الرئيسة | ✅ | 2026-09-21 | `b0ebe39` · DDL يدوي (Supabase Studio) |
