...
- **القرار:**
  1. **طبقة واحدة قابلة للإطفاء في `index.html`:** كل تعديلات العرض تحت `html.nd96`، ويتحكم بها الكائن `ND96` في `<head>`:
     ...
     - `ND96.endsAt = 2026-09-29T23:59:59+03:00` انتهاء ذاتي؛ عند الصفر تعاد تحميل الصفحة فتعود كالإنتاج، بلا خطوط العرض ولا صوره.
...
- **النتيجة:**
  ...
  - طبقة العرض تبقى في الكود خاملة بعد 29 سبتمبر إلى أن تُحذف بقرار لاحق.
...
| F24 | **عرض اليوم الوطني 96 على الصفحة الرئيسة** (ADR-015): `index.html` صار صفحة العرض بطبقة قابلة للإطفاء تنتهي ذاتياً 2026-09-29، والتسجيل يحفظ الباقة في `interests.period` (`nd96_m1/m2/m3`) بعد توسيع القيد، ولوحة التحكم تعرض أسماء الباقات، و`/index96.html` يُحوَّل إلى الرئيسة | ✅ | 2026-09-21 | `b0ebe39` · DDL يدوي (Supabase Studio) |
